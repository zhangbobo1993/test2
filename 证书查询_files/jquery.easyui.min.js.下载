...
(function($) {
	function _852(_853) {
		...
		function _855() {
			...
			$("<a href=\"javascript:void(0)\" class=\"datebox-clean\"></a>")
					.html(opts.cleanText).appendTo(_858);
			_858.find(".datebox-current,.datebox-clean").hover(function() {
			...
			_858.find(".datebox-current").click(function() {
				...
				if (nextNum == 1) {
					...
					// datetimebox 日期选择跳转到今天
					_854.calendar.calendar({
						year : new Date().getFullYear(),
						month : new Date().getMonth() + 1,
						current : new Date()
					});
					// 下面这一段先注释 下面的方法虽然能直接把当前日期的值赋到输入框
					// 但日期的格式是固定死的(yyyy-MM-dd hh:mm:ss) 导致重写formatter在点今天时无效
					/*
					 * var y1=date.getFullYear()+""; var
					 * m1=(date.getMonth()+1)+""; var d1=date.getDate()+""; var
					 * h1=date.getHours()+""; var M1=date.getMinutes()+""; var
					 * s1=date.getSeconds()+""; if(h1<"10"){ h1="0"+h1; }if(M1<"10"){
					 * M1="0"+M1; }if(s1<"10"){ s1="0"+s1; } var
					 * str1=y1+"-"+m1+"-"+d1+" "+h1+":"+M1+":"+s1;
					 * $(_853).combo("setValue",str1).combo("setText",str1);//设置空值
					 * $(_853).combo("hidePanel");
					 */
				}
			});
